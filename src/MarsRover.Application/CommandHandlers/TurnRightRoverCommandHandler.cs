...
{
    public class TurnRightRoverCommandHandler : IRequestHandler<TurnRightRoverCommand>
    {
        private readonly IUnitOfWork _unitOfWork;
        private const string roverNotLandedMessage = "The rover has not yet landed on the planet.";

        public TurnRightRoverCommandHandler(IUnitOfWork unitOfWork)
        {
            _unitOfWork = unitOfWork;
        }
        public async Task<Unit> Handle(TurnRightRoverCommand request, CancellationToken cancellationToken)
        {
